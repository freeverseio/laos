...
#[test]
fn on_runtime_upgrade_migration_works() {
	use crate::migration::cleanup_proposals::Migration;
	use frame_support::traits::OnRuntimeUpgrade;
	ExtBuilder::default().build().execute_with(|| {
		// In the mock, AccountId = u128 and Balance = u64
		let proposal = Proposal { proposer: 1, value: 1, beneficiary: 2, bond: 0 };
		Proposals::<Test>::insert(1, proposal);

		// After creating the proposal, we can find it in storage
		let value = Proposals::<Test>::iter().count();
		assert_eq!(value, 1);

		// Apply the migration
		parameter_types! {
			/// Mock weight for balance unreservations
			pub BalanceUnreserveWeight: Weight = Weight::from_parts(18_890_000, 0)
					.saturating_add(Weight::from_parts(0, 3593));
		}
		Migration::<Test, (), BalanceUnreserveWeight>::on_runtime_upgrade();

		// After the migration, the proposal has been erased
		let value = Proposals::<Test>::iter().count();
		assert_eq!(value, 0);
	});
}
