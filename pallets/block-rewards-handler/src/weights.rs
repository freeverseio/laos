
//! Autogenerated weights for `pallet_block_rewards_handler`
//!
//! THIS FILE WAS AUTO-GENERATED USING THE SUBSTRATE BENCHMARK CLI VERSION 4.0.0-dev
//! DATE: 2024-03-06, STEPS: `50`, REPEAT: `20`, LOW RANGE: `[]`, HIGH RANGE: `[]`
//! WORST CASE MAP SIZE: `1000000`
//! HOSTNAME: `trujideb`, CPU: `12th Gen Intel(R) Core(TM) i5-12500H`
//! WASM-EXECUTION: `Compiled`, CHAIN: `None`, DB CACHE: `1024`

// Executed Command:
// ./target/release/laos-ownership
// benchmark
// pallet
// --pallet
// pallet-block-rewards-handler
// --extrinsic=*
// --output
// pallets/block-rewards-handler/src/weights.rs
// --execution
// wasm
// --wasm-execution=compiled
// --template
// ./.maintain/frame-weight-template.hbs
// --steps
// 50
// --repeat
// 20

#![cfg_attr(rustfmt, rustfmt_skip)]
#![allow(unused_parens)]
#![allow(unused_imports)]
#![allow(missing_docs)]

use frame_support::{traits::Get, weights::{Weight, constants::RocksDbWeight}};
use core::marker::PhantomData;

/// Weight functions needed for `pallet_block_rewards_handler`.
pub trait WeightInfo {
	fn send_rewards() -> Weight;
}

/// Weights for `pallet_block_rewards_handler` using the Substrate node and recommended hardware.
pub struct SubstrateWeight<T>(PhantomData<T>);
impl<T: frame_system::Config> WeightInfo for SubstrateWeight<T> {
	/// Storage: `BlockRewardsHandler::RewardsAccount` (r:1 w:0)
	/// Proof: `BlockRewardsHandler::RewardsAccount` (`max_values`: Some(1), `max_size`: Some(20), added: 515, mode: `MaxEncodedLen`)
	/// Storage: `System::Account` (r:2 w:2)
	/// Proof: `System::Account` (`max_values`: None, `max_size`: Some(116), added: 2591, mode: `MaxEncodedLen`)
	fn send_rewards() -> Weight {
		// Proof Size summary in bytes:
		//  Measured:  `283`
		//  Estimated: `6172`
		// Minimum execution time: 37_350_000 picoseconds.
		Weight::from_parts(38_213_000, 6172)
			.saturating_add(T::DbWeight::get().reads(3_u64))
			.saturating_add(T::DbWeight::get().writes(2_u64))
	}
}

// For backwards compatibility and tests.
impl WeightInfo for () {
	/// Storage: `BlockRewardsHandler::RewardsAccount` (r:1 w:0)
	/// Proof: `BlockRewardsHandler::RewardsAccount` (`max_values`: Some(1), `max_size`: Some(20), added: 515, mode: `MaxEncodedLen`)
	/// Storage: `System::Account` (r:2 w:2)
	/// Proof: `System::Account` (`max_values`: None, `max_size`: Some(116), added: 2591, mode: `MaxEncodedLen`)
	fn send_rewards() -> Weight {
		// Proof Size summary in bytes:
		//  Measured:  `283`
		//  Estimated: `6172`
		// Minimum execution time: 37_350_000 picoseconds.
		Weight::from_parts(38_213_000, 6172)
			.saturating_add(RocksDbWeight::get().reads(3_u64))
			.saturating_add(RocksDbWeight::get().writes(2_u64))
	}
}