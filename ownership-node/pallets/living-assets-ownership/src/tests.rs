...
mod test {
	...
	fn living_assets_ownership_trait_create_new_collection() {
		new_test_ext().execute_with(|| {
        	let result = <LivingAssetsModule as LivingAssetsOwnership<AccountId, CollectionId>>::create_collection(0);
        	assert_ok!(result);
        	assert_eq!(LivingAssetsModule::owner_of_collection(0), Some(1));
    	});
	}
	...
	fn living_assets_ownership_trait_create_an_existing_collection_should_fail() {
		new_test_ext().execute_with(|| {
			assert_ok!(<LivingAssetsModule as LivingAssetsOwnership<AccountId, CollectionId>>::create_collection(0));
			assert_noop!(
				<LivingAssetsModule as LivingAssetsOwnership<AccountId, CollectionId>>::create_collection(0),
				Error::<Test>::CollectionAlreadyExists
			);
		});
	}
