...
import { CONTRACT_ADDRESS, GAS_LIMIT, GAS_PRICE, GENESIS_ACCOUNT, GENESIS_ACCOUNT_PRIVATE_KEY, LAOS_EVOLUTION_ABI, COLLECTION_EVOLVER_AND_MINTER_ABI, MAX_U96, RPC_PORT } from "./config";
...
export async function createCollection(context: { web3: Web3 }): Promise<Contract> {
	...
	const collectionContract = new context.web3.eth.Contract(COLLECTION_EVOLVER_AND_MINTER_ABI, result.events.NewCollection.returnValues._collectionAddress, {
		from: GENESIS_ACCOUNT,
		gas: GAS_LIMIT,
		gasPrice: GAS_PRICE,
	});
