...
use crate::{
	...
	pallet::{
		BalanceOf, CandidateInfo, Config, DelegationScheduledRequests, DelegatorState, Error, Event, Pallet, Round,
		RoundIndex, Total,
	},
...
impl<A, B> From<ScheduledRequest<A, B>> for CancelledScheduledRequest<B> {
	fn from(request: ScheduledRequest<A, B>) -> Self {
		CancelledScheduledRequest { when_executable: request.when_executable, action: request.action }
	}
}
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn delegation_cancel_request(
		...
		let request = Self::cancel_request_with_state(&delegator, &mut state, &mut scheduled_requests)
		...
		match request.action {
			DelegationAction::Revoke(amount) => {
				...
				if leaving {
					...
					Self::deposit_event(Event::DelegatorLeft { delegator, unstaked_amount: amount });
				} else {
					...
					if bond.owner == collator {
						return if bond.amount > amount {
							...
							state.total_sub_if::<T, _>(amount, |total| {
								...
								ensure!(new_total >= T::MinDelegation::get(), <Error<T>>::DelegationBelowMin);
								ensure!(new_total >= T::MinDelegatorStk::get(), <Error<T>>::DelegatorBondBelowMin);
	...
	pub(crate) fn delegator_schedule_revoke_all(delegator: T::AccountId) -> DispatchResultWithPostInfo {
		let mut state = <DelegatorState<T>>::get(&delegator).ok_or(<Error<T>>::DelegatorDNE)?;
		...
		<DelegatorState<T>>::insert(delegator.clone(), state);
		Self::deposit_event(Event::DelegatorExitScheduled { round: now, delegator, scheduled_exit: when });
		Ok(().into())
	}
	...
	pub(crate) fn delegator_cancel_scheduled_revoke_all(delegator: T::AccountId) -> DispatchResultWithPostInfo {
		let mut state = <DelegatorState<T>>::get(&delegator).ok_or(<Error<T>>::DelegatorDNE)?;
		let mut updated_scheduled_requests = vec![];
		...
		for bond in &state.delegations.0 {
			...
			scheduled_requests
				.iter()
				.find(|req| req.delegator == delegator && matches!(req.action, DelegationAction::Revoke(_)))
	...
	pub(crate) fn delegator_execute_scheduled_revoke_all(
		...
		if let DelegatorStatus::Leaving(when) = state.status {
			...
			for bond in state.delegations.0.clone() {
				if let Err(error) = Self::delegator_leaves_candidate(bond.owner.clone(), delegator.clone(), bond.amount)
				{
					log::warn!("STORAGE CORRUPTED \nDelegator leaving collator failed with error: {:?}", error);
				}
		...
		for bond in &state.delegations.0 {
			...
			let request_idx = scheduled_requests
				.iter()
				.position(|req| req.delegator == delegator && matches!(req.action, DelegationAction::Revoke(_)))
				.ok_or(<Error<T>>::DelegatorNotLeaving)?;
			let request = &scheduled_requests[request_idx];
		...
		for (bond, mut scheduled_requests, request_idx) in validated_scheduled_requests {
			...
			if let Err(error) = Self::delegator_leaves_candidate(collator.clone(), delegator.clone(), bond.amount) {
				log::warn!(
	...
	pub fn delegation_request_exists(collator: &T::AccountId, delegator: &T::AccountId) -> bool {
		<DelegationScheduledRequests<T>>::get(collator).iter().any(|req| &req.delegator == delegator)
	}
	...
	pub fn delegation_request_revoke_exists(collator: &T::AccountId, delegator: &T::AccountId) -> bool {
		<DelegationScheduledRequests<T>>::get(collator)
			.iter()
			.any(|req| &req.delegator == delegator && matches!(req.action, DelegationAction::Revoke(_)))
	}
}
...
mod tests {
	...
	fn test_cancel_request_with_state_removes_request_for_correct_delegator_and_updates_state() {
		...
		let mut scheduled_requests = vec![
			...
			ScheduledRequest { delegator: 2, when_executable: 1, action: DelegationAction::Decrease(50) },
		];
		let removed_request = <Pallet<Test>>::cancel_request_with_state(&1, &mut state, &mut scheduled_requests);

		assert_eq!(
			removed_request,
			Some(ScheduledRequest { delegator: 1, when_executable: 1, action: DelegationAction::Revoke(100) })
		);
		assert_eq!(
			scheduled_requests,
			vec![ScheduledRequest { delegator: 2, when_executable: 1, action: DelegationAction::Decrease(50) },]
		);
		assert_eq!(
			state,
			Delegator {
