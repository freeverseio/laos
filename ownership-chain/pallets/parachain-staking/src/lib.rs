...
//! ### Terminology
//!
//! - **Candidate:** A user which freezes up tokens to be included into the set of authorities which
//!   author blocks and receive rewards for doing so.
//!
//! - **Collator:** A candidate that was chosen to collate this round.
//!
//! - **Delegator:** A user which freezes up tokens for collators they trust. When their collator
...
//!
//! - **Freeze:** A freeze on a specified amount of an account's free balance until a specified
//!   block number. Multiple freezes always operate over the same funds, so they "overlay" rather
//!   than "stack"
//!
//! ## Genesis config
//!
...
pub mod pallet {
	...
	use crate::{
		set::OrderedSet,
		types::{
			BalanceOf, Candidate, CandidateOf, CandidateStatus, CreditOf, DelegationCounter,
			Delegator, RoundInfo, Stake, StakeOf, TotalStake,
		},
	};
	...
	impl<T: Config> Pallet<T> {
			...
			CandidatePool::<T>::iter().for_each(|(id, state)| {
				...
				state.delegators.into_iter().for_each(|delegator_state| {
					Self::do_inc_delegator_reward(
						&delegator_state.owner,
						delegator_state.amount,
						&id,
					);
			...
			if let Some((maybe_old_idx, top_candidates)) = maybe_top_candidate_update {
				...
				match (was_collating, is_collating) {
					...
					(false, true) => {
						...
						let (drop_self, drop_delegators) = match max_selected_candidates
							.cmp(&top_candidates.len())
						{
							// top candidates are not full
							Ordering::Greater => (BalanceOf::<T>::zero(), BalanceOf::<T>::zero()),
							// top candidates are full. the collator with the lowest stake is at
							// index old_col_idx
							_ => {
								// we can unwrap here without problems, since we compared
								// [max_selected_candidates] with [top_candidates] length, but lets
								// be safe.
								Self::get_top_candidate_stake_at(
									&top_candidates,
		...
		fn increase_lock(
			...
			more: BalanceOf<T>,
				...
				pallet_balances::Pallet::<T>::reducible_balance(
					who,
		...
		fn do_unlock(who: &T::AccountId) -> Result<u32, DispatchError> {
			...
			total_freezed = if let Some(IdAmount { amount, .. }) = freezes.iter().find(|l| {
				l.id == <T as pallet::Config>::FreezeIdentifier::from(FreezeReason::Staking).into()
			}) {
		...
		fn note_author(author: T::AccountId) {
			...
			if CandidatePool::<T>::get(&author).is_some() {
				...
				BlocksAuthored::<T>::mutate(&author, |count| {
					*count =
						count.saturating_add(authors.len().saturated_into::<BlockNumberFor<T>>());
				});
			}
