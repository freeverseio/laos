...
This is the simplest path to renewing LAOS coretime. You only need a browser and the LAOSFOUNDATION account in a wallet extension.
...
- [ ] **The LAOSFOUNDATION account imported** into that extension (`1411FgYGWbhkFX6e1Nx36oysyDQ54bzyRxKWWojuVN8VD6rU`). You'll need its seed phrase or JSON backup to import it.
- [ ] **At least 11 DOT available** in that account on the **Polkadot Relay Chain** (10.3 DOT for the renewal + ~0.5 DOT for transaction fees). If the DOT is already on the Coretime Chain, skip Step 2.
...
### Step 1 — Install a wallet and import the account

1. Go to [talisman.xyz](https://talisman.xyz) and install the browser extension.
2. Open Talisman → click **"Add Account"** → **"Import via Recovery Phrase"** (or Import JSON).
3. Enter the seed phrase for the LAOSFOUNDATION account. The address `1411FgYGWbhkFX6e1Nx36oysyDQ54bzyRxKWWojuVN8VD6rU` should appear in your wallet.
4. Make sure the account shows DOT balance. If it shows zero, the DOT may be on a different account.
...
The `broker.renew()` extrinsic on the Polkadot Coretime Chain is a **permissionless call** — technically any account with sufficient DOT can submit it for any ParaId. However, convention and practical control are structured as follows:

### The LAOS Foundation Account (Primary)

The LAOS parachain was registered by:

```
1411FgYGWbhkFX6e1Nx36oysyDQ54bzyRxKWWojuVN8VD6rU  (LAOSFOUNDATION)
```

This is the **manager account** on-chain for ParaId 3370 and is the natural owner of the renewal action. Any person controlling the private key(s) for this account (or the multisig/HSM that backs it) can submit the renewal.

### Governance Path (fallback)

Because pallet_sudo has been removed from LAOS, on-chain governance controls root-level actions. However, `broker.renew()` does **not** require root — it requires only a signed origin with enough DOT. The governance path is therefore not needed for coretime renewal.

For reference, LAOS governance structure is:
- **Council** (majority / 2/3 / all) — controls external proposals and treasury
- **Technical Committee** — can fast-track referenda (3h voting, 6min enactment)
- **Democracy** — standard public referenda with 7-day voting period

If the foundation account is inaccessible, a council motion can be used to route treasury funds toward renewal via a governance proposal — but this path takes at minimum ~3 hours even with a fast-track, so it should not be relied upon as a last-minute option.
...
- Access to account `1411FgYGWbhkFX6e1Nx36oysyDQ54bzyRxKWWojuVN8VD6rU` (or another account with sufficient DOT)
- Sufficient DOT for the coretime cost — the **current locked-in renewal price for LAOS is 10.3 DOT** (verified on-chain April 20, 2026; it increases 3% each 28-day period)
- A browser with [Polkadot.js Apps](https://polkadot.js.org/apps) or [Talisman](https://talisman.xyz) wallet
...
2. Select the signing account: `1411FgYGWbhkFX6e1Nx36oysyDQ54bzyRxKWWojuVN8VD6rU`
3. Select pallet: **`broker`**
4. Select extrinsic: **`renew`**
5. Fill in the parameter:
   - `core`: **`8`**
6. Click **Submit Transaction** and sign with the account's key

The transaction will:
- Deduct the coretime price in DOT from the signing account
- Register a new 28-day bulk coretime assignment for ParaId 3370 on Core 8
- Take effect at the next timeslice boundary, ensuring seamless continuity
...
| **ASAP** | Submit `broker.renew(8)` from the LAOSFOUNDATION account |
...
| Who can submit it? | Any account with enough DOT — primary is `1411FgYGWbhkFX6e1Nx36oysyDQ54bzyRxKWWojuVN8VD6rU` (LAOSFOUNDATION) |
