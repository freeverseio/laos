...
{
	// Build the partial components of the service using the provided configuration and Ethereum RPC
	// configuration. This includes the client, backend, task manager, import queue, keystore,
	// select chain, transaction pool, and other components.
	...
	// Create the full network configuration based on the service configuration.
	...
	//     // Start the Aura consensus for block production.
	...
	//             // Closure to create inherent data providers for block production.
	...
	//                         slot_duration,
	//                     );
	//                 // Create mocked parachain inherent data provider.
	...
	//         // Set block proposal timings: We have around 500ms for proposing.
	...
	//     // The Aura authoring task is considered essential; if it fails, the service will shut
	// down.     task_manager
	//         .spawn_essential_handle()
	//         .spawn_blocking("aura", Some("block-authoring"), aura);
	// } else {
	//     // If not running as an authority, log a warning message.
	//     log::warn!("You could add --alice or --bob to make dev chain seal instantly.");
	// }

	// // Clone the Prometheus registry from the configuration.
	// let prometheus_registry = config.prometheus_registry().cloned();
	// // Create storage overrides for Frontier (Ethereum compatibility layer).
	// let overrides = fc_storage::overrides_handle(client.clone());
	// // Create a cache for Ethereum block data to improve RPC performance.
	...
	// // Initialize Ethereum pubsub notification sinks (used for RPC pubsub).
	// let pubsub_notification_sinks: fc_mapping_sync::EthereumBlockNotificationSinks<
	//     fc_mapping_sync::EthereumBlockNotification<Block>,
	// > = Default::default();
	// let pubsub_notification_sinks = Arc::new(pubsub_notification_sinks);

	// // For Ethereum-compatibility RPC, set the RPC ID provider.
	// config.rpc_id_provider = Some(Box::new(fc_rpc::EthereumSubIdProvider));

	// // Spawn Frontier tasks (Ethereum compatibility layer), including tracing tasks if enabled.
	...
	// // Build the RPC extensions builder.
	...
	//     // Define a closure to create inherent data providers for pending transactions.
	...
	//     // Return a boxed closure that builds the RPC extensions.
	//     Box::new(move |deny_unsafe, subscription_task_executor| {
	//         // Build the dependencies for the RPC.
	...
	//         // Check if tracing APIs are enabled in the Ethereum RPC configuration.
	...
	//         {
	//             // Create the full RPC with tracing enabled.
	...
	//             .map_err(Into::into)
	//         } else {
	//             // Create the full RPC without tracing.
	//             crate::rpc::create_full::<_, _, _, _, crate::rpc::DefaultEthConfig<_, _>, _>(
	//                 deps,
	//                 subscription_task_executor,
	//                 pubsub_notification_sinks.clone(),
	...
	// // Spawn essential tasks for the service, including the RPC server and other background
	// tasks. sc_service::spawn_tasks(sc_service::SpawnTasksParams {
	...
	// // Start the network, allowing the node to connect to peers.
	// start_network.start_network();

	// Return the task manager to keep the service running.
	Ok(task_manager)
}
