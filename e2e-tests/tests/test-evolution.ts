...
describeWithExistingNode("@qa Frontier RPC (Public Minting)", (context) => {
	...
	step("public minting is disabled by default and when is deactivated event is emitted", async function () {
		...
		const disablingPublicMintingResult = await collectionContract.methods
		...
		expect(disablingPublicMintingResult.events.PublicMintingDisabled.raw.topics[0]).to.be.eq(
			SELECTOR_LOG_PUBLIC_MINTING_DISABLED
		);
	...
	step("enable public minting emits an event", async function () {
		const enablingPublicMintingResult = await collectionContract.methods
			.enablePublicMinting()
			.send({ from: FAITH, gas: GAS_LIMIT });
		...
		expect(enablingPublicMintingResult.events.PublicMintingEnabled.raw.topics[0]).to.be.eq(
			SELECTOR_LOG_PUBLIC_MINTING_ENABLED
		);
	...
	step("I can mint even I'm not the owner", async function () {
		...
		const mintingResult = await collectionContract.methods
			.mintWithExternalURI(ALITH, "123", "some/random/token/uri")
			.send({ from: ALITH, gas: GAS_LIMIT, nonce: nonce++ });
		expect(mintingResult.status).to.be.eq(true);
	});
	...
	step("after changing owner I can't disable", async function () {
		await collectionContract.methods
			.transferOwnership("0xf24FF3a9CF04c71Dbc94D0b566f7A27B94566cac")
			.send({ from: FAITH, gas: GAS_LIMIT });
		try {
