...
describeWithExistingNode("Frontier RPC (Create Collection)", (context) => {
	...
	step("when collection is created, it should return owner", async function () {
		...
		expect(owner).to.be.eq(TEST_ACCOUNT);
	});

	step("when collection is created event is emitted", async function () {
		this.timeout(70000);

		const result = await contract.methods.createCollection(TEST_ACCOUNT).send({
			from: TEST_ACCOUNT,
			gas: GAS_LIMIT,
			gasPrice: GAS_PRICE,
		});
		...
		expect(result.events.NewCollection.returnValues._owner).to.be.eq(TEST_ACCOUNT);
	...
	step("when collection is created, bytecode is inserted in the storage", async function () {
		...
		expect(await context.web3.eth.getCode(TEST_ACCOUNT)).to.be.eq("0x");
	});
	...
	step("create collection call can estimate gas", async function () {
		...
		const estimatedGas = await contract.methods.createCollection(TEST_ACCOUNT).estimateGas({
			from: TEST_ACCOUNT,
			gas: GAS_LIMIT,
			gasPrice: GAS_PRICE,
			nonce: nonce++,
