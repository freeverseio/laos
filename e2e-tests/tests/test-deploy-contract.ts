...
import { ALITH, ALITH_PRIVATE_KEY, BALANCES_ABI, BALANCES_BYTECODE, GAS_PRICE } from "./config";
...
describeWithExistingNode("Frontier RPC (Deploying contract)", (context) => {
    it("", async function () {
        ...
        const estimatedGas = await contract.deploy(deployOptions).estimateGas(); // en moonbeam 1861715
