...
import { TESTING_ACCOUNT, TESTING_ACCOUNT_PRIVATE_KEY } from "./config";
...
describeWithExistingNode("Frontier RPC (TxPoolApi)", (context) => {
	...
	before(async function () {
		nonce = await context.web3.eth.getTransactionCount(TESTING_ACCOUNT);
	});
	...
	step("txpool_content should return correct result", async function () {
		...
		let genesisAccount = TESTING_ACCOUNT.toLowerCase();
