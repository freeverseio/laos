...
import { TESTING_ACCOUNT2, TESTING_ACCOUNT2_PRIVATE_KEY } from "./config";
...
describeWithExistingNode("Frontier RPC (TxPoolApi)", (context) => {
	...
	before(async function () {
		nonce = await context.web3.eth.getTransactionCount(TESTING_ACCOUNT2);
	});
	...
	step("txpool_content should return correct result", async function () {
		...
		let genesisAccount = TESTING_ACCOUNT2.toLowerCase();
