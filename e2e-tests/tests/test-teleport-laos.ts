...
describeWithExistingNode(
	...
	(context) => {
		...
		step("Create $LAOS in AssetHub", async function () {
			const laosForeignAssetExists = !(await apiAssetHub.query.foreignAssets.asset(apiAssetHub.laosAssetId))
				.isEmpty;
			...
			if (!laosForeignAssetExists) {
				...
				const laosBalanceBefore = new BN(
					(await apiAssetHub.query.system.account(laosSiblingAccount)).data.free
				);
		...
		step("Mint $LAOS in AssetHub", async function () {
			...
			const ferdieXLaosBalance = hexToBn(
				(await apiAssetHub.query.foreignAssets.account(apiAssetHub.laosAssetId, ferdie.address)).toJSON()[
					"balance"
				]
			);
